...
<div class="mx-auto max-w-4xl px-4 py-6">
	...
	<Card variant="static" surface={40} padding="p-5" class="mb-6">
		...
		{#if !online}
			<div class="mt-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-400">📡 Hors ligne — la génération nécessite une connexion à NotebookLM</div>
		{/if}
		<div class="mt-4 flex flex-wrap gap-2">
			{#each Object.entries(typeMeta) as [type, meta]}
				<button disabled={!online || generating !== null} onclick={() => gen(type)}
					class="inline-flex items-center gap-1.5 rounded-lg border border-coal-700 bg-coal-900 px-3 py-1.5 text-xs text-coal-300 transition-colors hover:border-ember-500 hover:text-ember-400 disabled:opacity-40 disabled:cursor-not-allowed">
					...
					<div class="flex items-center justify-between">
						...
						{#if a.file_path}
							<a href={a.file_path} target="_blank"
								class="rounded-lg border border-coal-700 bg-coal-900 px-3 py-1.5 text-xs text-coal-300 transition-colors hover:border-ember-500 hover:text-ember-400">Télécharger</a>
						{/if}
					</div>
				</Card>
			{/each}
	...
	{:else}
		<Card variant="static" surface={40} padding="p-8" class="text-center">
			<p class="text-coal-500">Aucun artifact pour ce voyage.</p>
		</Card>
	{/if}
</div>
